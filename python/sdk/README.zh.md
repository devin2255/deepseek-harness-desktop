...
`DeepSeekHarness` 和 `HarnessClient` 提供 `list_tasks()`、`define_task()`、`update_task_criterion()`、`record_task_risk()` 与 `review_task()`。Pydantic 会校验每个 Task 基线和已提交完整行，同时精确保留协议中的标识与判别字段；无效响应会抛出 `SdkProtocolError`。这些调用不在 SDK 侧维护 Task 缓存。

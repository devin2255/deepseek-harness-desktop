...
`DeepSeekHarness` and `HarnessClient` expose `list_tasks()`, `define_task()`, `update_task_criterion()`, `record_task_risk()`, and `review_task()`. Pydantic validates each Task baseline and committed row while preserving wire identifiers and discriminants exactly; invalid responses raise `SdkProtocolError`. These calls keep no SDK-side Task cache.

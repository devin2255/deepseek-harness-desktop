...
The `web`, `desktop`, and `headless` profiles auto-initialize from shipped templates on first use (`web`: base + web-app; `desktop`: base + web-app + desktop-app; `headless`: base + headless). The desktop profile starts only under its supervising Electron launcher, which supplies its per-launch capability; it is not a direct user invocation. Any other missing profile fails loud with a hint to run `dsh plugin --profile <name> add <package>`.
...
| `desktop` | supervising Electron launcher only; not a direct user invocation |

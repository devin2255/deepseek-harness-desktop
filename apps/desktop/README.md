...
Only a packaged Windows application with the reviewed GitHub update source and a signing publisher enables update checks; unsigned test packages and source runs do not. Main checks shortly after startup and every six hours while running, and the tray can request another check. A newer release downloads without interrupting Tasks, but never installs on ordinary quit. When the signed installer has downloaded, the tray and a native notification offer installation. The confirmation defaults to Later and states the current Task activity or that activity is unavailable. Approval stops background presence and Harness, releases the application mutex, and only then launches the assisted update installer; incomplete cleanup prevents the installer launch. Check and download failures remain retryable from the tray and are recorded in the desktop log. See the [update decision](../../.agents/notes/implemented/feature/2026-09-24-desktop-consented-update-installation.md).
...
The [Windows installer workflow](../../.github/workflows/desktop-installer.yml) runs the complete installer suite for pull requests, master, and `dsh-v*` or `v*` pushes on a fresh hosted Windows runner. It retains package-validated EXE, checksum, and metadata files for 30 days, including when a later acceptance test fails; check the run's test result before using an artifact. This workflow has no signing credentials and does not publish a production release.
...
The [production desktop release workflow](../../.github/workflows/desktop-release.yml) is a separate manual operation from a selected `v<version>` tag reachable from `master`; `dsh-v<version>` remains the npm release tag. Its protected `desktop-release` environment must provide `WIN_CSC_LINK` and `WIN_CSC_KEY_PASSWORD`. It requires valid Authenticode and the same signer on the installer and installed application, repeats the complete installer matrix, creates GitHub build provenance, and only then publishes the EXE, checksum, release metadata, and `latest.yml` as GitHub Release assets. Selecting the workflow with `publish` disabled performs no release job.

## Known Limitations

- **Installer qualification** — Windows lifecycle qualification, including an upgrade from a separately built older signed release, is required before distribution; unsigned local builds can trigger SmartScreen, while production publication requires the protected signing environment and an explicit manual decision. The updater requires a signed Windows package, so end-to-end download and installation still require signed-release acceptance. macOS Gatekeeper qualification, signing, notarization, and updates are not implemented.

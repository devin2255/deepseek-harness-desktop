...
这是根任务、应用所有的执行 Worktree、验收标准、证据、风险、评审决策和统一注意力行的持久词汇与服务定义。任务事实使用带严格回放校验的全值 Session 事件，因此恢复不依赖进程本地状态。Worktree 分配只记录一次源 Workspace、创建提交、源目录脏状态摘要、分支和执行路径。证据指向根任务树中的准确事件序号。

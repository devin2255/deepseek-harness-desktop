...
The optional `shell.home` and `shell.review` slots add full center workspaces without replacing the conversation registration or its child slots. The layout starts on home when that slot is occupied; otherwise it shows the ordinary conversation. `ctx.layout.showHome()`, `showConversation()`, and `showReview()` change only presentation state and emit `layout/navigate` on every call, including repeated destinations, so pending navigation can be cancelled. The hidden conversation remains mounted, retaining drafts and selection, while home and Review render details at zero width without changing its saved preference. Sidebar and workspace gestures explicitly reveal the conversation even when the selected session id stays unchanged.

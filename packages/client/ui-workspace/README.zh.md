...
选择会话、新建会话和成功的 fork 导航会显式显示布局的会话页面，包括在可选首页可见时选择已经是当前项的会话；fork 失败时保留当前页面与选择。
...
当前 Task 拥有应用管理的执行 worktree 时，选中的 picker 行会保留源 Workspace 标题并追加“工作树”。该标签以权威 Task 投影为依据；picker 不会根据 Session cwd 推断隔离状态。

...
根任务投影使用一套独立的 Host 协议。`task.list` 提供当前 generation 下完整且有序的重连 baseline；`task.define`、`task.updateCriterion`、`task.recordRisk` 和 `task.review` 把带 compare-and-set 的变更转发给 `ctx.tasks`。后续整行变化通过 Host 流中的 `task/changed` 增量发送，帧内包含同一 generation 以及 `upserts` 和 `removed`，流的 disposer 会释放其对应的 Task 订阅。请求与响应 schema 会在数据越过 provider 边界前拒绝未知属性、首尾空白、无效 discriminant、负 sequence、重复 criterion identity 和格式错误的 evidence。每个 `TaskError` code 都映射为稳定的小写 RPC code；未组合 Task 服务的部署返回 `task-unavailable`，且不会发送 Task 增量。

...
export const rpcErrorSchema: z.ZodType<RpcError> = z.discriminatedUnion('code', [
  ...
  z.object({ code: z.literal('workspace-isolation-unavailable'), message: z.string(), details: z.object({
    workspaceId: z.string().optional(),
    sessionId: z.string().optional(),
    worktreeCode: z.enum([
      'WORKTREE_NOT_GIT', 'WORKTREE_NESTED_REPOSITORY', 'WORKTREE_UNBORN_HEAD', 'WORKTREE_INSUFFICIENT_SPACE',
      'WORKTREE_TARGET_OCCUPIED', 'WORKTREE_BRANCH_OCCUPIED', 'WORKTREE_GIT_FAILED', 'WORKTREE_UNAVAILABLE',
    ]).optional(),
  ...
  z.object({ code: z.literal('task-not-found'), message: z.string(), details: z.object({ sessionId: z.string() }) }),
  z.object({ code: z.literal('task-target-not-root'), message: z.string(), details: z.object({ sessionId: z.string() }) }),
  z.object({ code: z.literal('task-stale-sequence'), message: z.string(), details: z.object({ sessionId: z.string() }) }),
  z.object({ code: z.literal('task-invalid-definition'), message: z.string(), details: z.object({ sessionId: z.string() }) }),
  z.object({ code: z.literal('task-invalid-criterion'), message: z.string(), details: z.object({ sessionId: z.string() }) }),
  z.object({ code: z.literal('task-invalid-risk'), message: z.string(), details: z.object({ sessionId: z.string() }) }),
  z.object({ code: z.literal('task-invalid-review'), message: z.string(), details: z.object({ sessionId: z.string() }) }),
  z.object({ code: z.literal('task-invalid-commit'), message: z.string(), details: z.object({ sessionId: z.string() }) }),
  z.object({ code: z.literal('task-invalid-apply'), message: z.string(), details: z.object({ sessionId: z.string() }) }),
  z.object({ code: z.literal('task-invalid-discard'), message: z.string(), details: z.object({ sessionId: z.string() }) }),
  z.object({ code: z.literal('task-review-unavailable'), message: z.string(), details: z.object({ sessionId: z.string() }) }),
  z.object({ code: z.literal('task-review-rejected'), message: z.string(), details: z.object({
    sessionId: z.string(),
    reviewCode: z.enum([
      'REVIEW_WORKTREE_UNAVAILABLE', 'REVIEW_WORKTREE_DIVERGED', 'REVIEW_STALE', 'REVIEW_INVALID_PATH',
      'REVIEW_FILE_NOT_FOUND', 'REVIEW_EMPTY', 'REVIEW_INCOMPLETE', 'REVIEW_INVALID_MESSAGE',
      'REVIEW_IDENTITY_MISSING', 'REVIEW_SOURCE_DIRTY', 'REVIEW_SOURCE_MOVED', 'REVIEW_APPLY_CONFLICT',
      'REVIEW_CONFIRMATION_REQUIRED', 'REVIEW_GIT_FAILED',
    ]),
  }) }),
  z.object({ code: z.literal('task-invalid-evidence'), message: z.string(), details: z.object({ sessionId: z.string() }) }),

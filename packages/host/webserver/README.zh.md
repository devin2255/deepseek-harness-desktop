...
- **不提供 TLS、内置认证或来源策略**：调用方注册的 request guard 可以执行放行控制，但绑定非回环地址会向对应网络公开服务器；面向部署的加固措施（或在前方放置真正的反向代理）有意不纳入面向开发环境的 v1。

...
[目录选择 seam](../directory-picker/README.md) 的**原生 OS 选择器后端**：`NativeDirectoryPicker` 以 `native` 能力注册 `ctx.directoryPicker`，其 `pick(signal)` 每次调用打开一个原生选择器并解析出所选绝对路径（取消时为 `null`）。平台工具不经 shell 调用：macOS 使用 `osascript`，Linux 使用 Zenity 并以 KDialog 回退；调用方的中止信号会终止原生进程。Windows 在 spawn 的子进程中打开现代 `IFileOpenDialog`——由 koffi 在子进程主线程上驱动的 COM 会话，采用宿主接受的最佳线程 DPI 感知（优先 per-monitor-v2），中止时向对话框线程投递 `WM_CLOSE`。子进程环境会强制启用 Electron 的 Node 模式，使普通 Node 宿主和带品牌信息的 Electron 可执行文件都运行打包的 worker 入口，而不是桌面应用入口。koffi 将 COM 持有的 UTF-16 结果编组为托管字符串，并通过 `CoTaskMemFree` 释放原内存；该路径不创建 Electron Node 模式所禁止的 external ArrayBuffer。IPC 通道在对话框显示前的 `showing` 通知后保持连接，只有所选路径、取消或错误终态结果完成发送后才会关闭。只有操作者坐在宿主屏幕前时才可用——远程部署应组合 [`-browse`](../directory-picker-browse/README.md)。命令边界（`DirectoryPickerRunner`）与平台事实可注入。共享的免 shell 子进程运行器位于 [`dsh-native-command`](../../util/native-command/README.md)。

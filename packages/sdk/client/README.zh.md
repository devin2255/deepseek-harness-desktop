...
两层客户端都提供 `listTasks`、`defineTask`、`updateTaskCriterion`、`recordTaskRisk` 和 `reviewTask`。它们返回运行时给出的分离 Task 基线或已提交完整行；`executionWorkspace` 会原样保留 Host 给出的源路径、Worktree 路径、分支和 Git 标识，不做路径改写。SDK 不维护 Task 缓存，格式错误的 Task 行或 Worktree 分配会抛出 `SdkProtocolError`。

...
两层客户端都提供 `listTasks`、`defineTask`、`updateTaskCriterion`、`recordTaskRisk` 和 `reviewTask`。它们返回运行时给出的分离 Task 基线或已提交完整行；SDK 不维护 Task 缓存，格式错误的 Task 响应会抛出 `SdkProtocolError`。

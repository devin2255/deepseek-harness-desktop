...
Both client layers expose `listTasks`, `defineTask`, `updateTaskCriterion`, `recordTaskRisk`, and `reviewTask`. They return detached Task baselines or committed rows from the runtime; the SDK owns no Task cache, and malformed Task responses raise `SdkProtocolError`.

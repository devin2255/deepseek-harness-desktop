...
Both client layers expose `listTasks`, `defineTask`, `updateTaskCriterion`, `recordTaskRisk`, and `reviewTask`. They return detached Task baselines or committed rows from the runtime; an `executionWorkspace` retains the Host-provided source path, worktree path, branch, and Git identity without rewriting. The SDK owns no Task cache, and malformed Task rows or worktree assignments raise `SdkProtocolError`.

...
Both client layers expose `listTasks`, `defineTask`, `updateTaskCriterion`, `recordTaskRisk`, and `reviewTask`. Isolated Tasks also expose `getTaskReviewSummary`, `getTaskReviewDiff`, `commitTask`, `applyTask`, and `discardTask`; every mutation carries the exact displayed review revision and Task sequence. They return detached Task baselines, bounded review values, or committed rows from the runtime; an `executionWorkspace` retains the Host-provided source path, worktree path, branch, and Git identity without rewriting. The SDK performs no Git operation, owns no Task cache, and validates Task rows, review files, diffs, and delivery receipts before returning them; malformed values raise `SdkProtocolError`.

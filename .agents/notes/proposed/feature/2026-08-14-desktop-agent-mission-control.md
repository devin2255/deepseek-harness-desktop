...
The secured desktop-foundation vertical slice is implemented: it provides the supervised Harness process, launch-scoped loopback authorization, sandboxed Renderer, bounded lifecycle, and a built real-Electron acceptance path described by the [desktop foundation decision](../../implemented/architecture/2026-08-14-electron-desktop-foundation.md). Task projection, application-owned worktrees, task-aware tray behavior, Mission Control UI, review, Harness Studio, signing, and updates remain unimplemented follow-on slices. They stay outside Electron Main and preload so those components do not acquire product-domain state.

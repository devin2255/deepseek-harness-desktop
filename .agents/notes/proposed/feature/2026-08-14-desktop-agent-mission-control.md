...
The shipped desktop foundation uses Electron because the Host, PTY stack, plugin runtime, and client build already run on Node and Chromium-compatible Web APIs. Electron Main supervises one Node `utilityProcess` running the desktop profile and one sandboxed Renderer running the existing plugin-composed React client. The frozen preload bridge exposes only the current platform identifier and one validated Main-to-Renderer Session-target subscription. The Renderer has context isolation and the Chromium sandbox enabled, with no Node integration or generic Electron IPC.

The shipped Harness process listens on an OS-assigned loopback port and requires a per-launch Bearer capability for every HTTP and WebSocket connection. An isolated Electron session injects the capability header only for that exact origin; navigation, redirects, new windows, and permissions outside the origin are denied. The value never enters Renderer or Preload APIs, URLs, logs, settings, or Session events. Electron Main owns single-instance application lifecycle, replaceable windows, supervised Harness startup and shutdown, and task-derived native presentation; product state remains in Cordis plugins.

The shipped task-aware background lifecycle keeps Harness running after the last window closes and recreates an authorized window from the native tray or a second launch. Electron Main polls authenticated Task and Session projections, retains only detached presentation values, and reports live or unavailable activity in one tray summary. New actionable attention, failure, and completion transitions create native notifications whose validated owner Session id routes through the existing client navigation controller. Explicit quit offers continue in the background, stop and quit, or cancel whenever activity is running or unavailable. The [Host crash-recovery decision](../../implemented/bug-fix/2026-09-21-desktop-host-crash-recovery.md) revokes dead process authority, requires explicit Retry, and projects a cold-repaired interrupted turn without replaying an unconfirmed tool call.

The secured desktop foundation, durable Task overview, application-owned root-task worktrees, review and delivery workspace, task-aware background presence, and abnormal-Host recovery are implemented. The foundation provides the supervised Harness process, launch-scoped loopback authorization, sandboxed Renderer, bounded lifecycle, and the built real-Electron acceptance path described by the [desktop foundation decision](../../implemented/architecture/2026-08-14-electron-desktop-foundation.md). The Task Service Definition owns branded acceptance, risk, and attention identities; whole-value Session events; strict replay; compare-and-set request values; and the detached list projection. Its Session-backed Provider reconstructs cold and live roots, preserves uninterrupted subagent ancestry, aggregates durable and generation-scoped attention, validates same-tree evidence, and serializes compare-and-set writes. The Host publishes its Agent and pending-question registries as one complete live-fact generation, exposes Task reads and mutations over Host RPC and both SDKs, and streams whole-row changes to a reconnect-safe client store. The [worktree decision](../../implemented/feature/2026-09-09-application-owned-task-worktrees.md) records default isolated creation and its failure guarantees. Child-writer integration, the broader Mission Control UI, Harness Studio, signing, updates, and macOS distribution remain follow-on slices. Product-domain state stays outside Electron Main and preload.

## Product structure

The [task overview plugin](../../../../packages/client/ui-task-overview/README.md) consumes the Task store rather than deriving a second domain model from conversation rows. It groups all six Task statuses, shows criteria progress, unresolved risks, active descendants, freshness, and every attention owner, and keeps the conversation mounted while hidden so returning to supervision does not reset selection or drafts. The client invalidates disconnected requests before accepting a fresh baseline; transport health remains connection-owned. Pure projection, navigation, component, keyless assembled, and real Electron acceptance cover concurrent roots in separate disposable workspaces, retained rows across transport reconnection, Renderer reload with stable Task and attention identities, and authoritative navigation to a child that owns a pending question.

The [parallel task overview specification](../../../../docs/superpowers/specs/2026-09-04-parallel-task-overview-design.md) scopes the first task-supervision slice. It reuses session activity and interaction facts without interpreting inactivity or unread reminders as successful delivery; the review workspace remains a separate acceptance target.

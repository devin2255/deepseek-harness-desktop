...
The secured desktop foundation and initial activity-and-attention overview are implemented. The foundation provides the supervised Harness process, launch-scoped loopback authorization, sandboxed Renderer, bounded lifecycle, and the built real-Electron acceptance path described by the [desktop foundation decision](../../implemented/architecture/2026-08-14-electron-desktop-foundation.md). The Task Service Definition owns branded acceptance, risk, and attention identities; whole-value Session events; strict replay; compare-and-set request values; and the detached list projection. Its Session-backed Provider now reconstructs cold and live roots, preserves uninterrupted subagent ancestry, aggregates durable and generation-scoped attention, validates same-tree evidence, and serializes compare-and-set writes. Host RPC, the client projection, outcome-and-review UI, application-owned worktrees, task-aware tray behavior, the broader Mission Control UI, Harness Studio, signing, and updates remain follow-on slices. Product-domain state stays outside Electron Main and preload.

## Product structure

The [task overview plugin](../../../../packages/client/ui-task-overview/README.md) derives activity and pending-owner navigation from existing metadata rather than another task database. The layout owns an additive home slot and keeps the conversation mounted while hidden, so returning to supervision does not reset selection or drafts. Both list owners invalidate disconnected requests before accepting fresh baselines, as documented in the [runtime list semantics](../../../../packages/client/runtime/README.md#workspace-and-session-lists); transport health remains connection-owned. Pure selectors, navigation and component tests, the built keyless navigation snapshot, and real Electron acceptance cover concurrent roots in separate disposable workspaces, retained rows across transport reconnection, and authoritative navigation to a child that owns a pending question.

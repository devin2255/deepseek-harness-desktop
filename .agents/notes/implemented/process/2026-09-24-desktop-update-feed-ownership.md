...
An installed Windows package contains an unambiguous update source, and a release manifest cannot silently name different installer bytes. Release creation requires `latest.yml` alongside the EXE. Signed old-to-new installation acceptance and signed macOS publication remain separate work; these files alone do not make an application self-updating.

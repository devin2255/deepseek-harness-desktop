...
`packages/host/directory-picker-native` opens `IFileOpenDialog` (`FOS_PICKFOLDERS | FOS_FORCEFILESYSTEM | FOS_NOCHANGEDIR`) in-process through koffi — already a workspace dependency for the repo's other `win32.ts` surfaces — as the primary win32 tier. The COM conversation runs in a spawned child process so the modal `Show` never blocks the host event loop; the child posts its native thread id before blocking, keeps IPC connected while `Show` blocks, and disconnects only after the selected-path, cancellation, or error result is flushed. The driver services aborts by re-posting `WM_CLOSE` to that thread's windows (`EnumThreadWindows`), killing the child when the close budget is exhausted. The child environment forces `ELECTRON_RUN_AS_NODE=1`; without that value, an electron-builder branded executable starts the desktop entry, observes the existing single-instance lock, and exits without joining the worker IPC channel. Koffi converts the COM-owned UTF-16 path through a disposable `str16` output and calls `CoTaskMemFree` after conversion; `koffi.view()` is not used because Electron forbids the external ArrayBuffer it creates. The dialog is the child's first window, so Windows activates it without a foreground call. The child thread opts into the best thread DPI awareness the host accepts (`SetThreadDpiAwarenessContext`, cascading per-monitor-v2 → per-monitor → system-aware with the return value checked), a strict upgrade over the script's system-DPI ceiling; DPI stays a cosmetic best-effort — a host accepting none of them still gets the modern dialog rather than a downgrade. The module split keeps coverage honest on every host: `win32-dialog-logic.ts` (pure sequencing) and `win32-dialog.ts` (driver) test against fakes anywhere; `win32-dialog-bindings.ts` tests against a mocked `koffi` COM world (the `dsh-session-persistence-jsonl` technique); POSIX hosts run the real spawn plumbing to its koffi-load rejection; win32 hosts run a real open-and-abort-close smoke. The PowerShell chain that preceded this tier is gone (see the [chain removal](../simplification/2026-08-04-drop-windows-powershell-picker-fallback.md)): the tier has no fallback.
...
- The binding test pins disposable UTF-16 output conversion without external buffers, the driver test pins the forced Node-mode child environment, and the worker test pins that `showing` cannot close IPC before the terminal result. The source plane and built `lib/worker.cjs` run under their real launchers. Automated acceptance still does not exercise a branded packaged executable opening and completing a folder selection; that remains a Windows installer acceptance gap.

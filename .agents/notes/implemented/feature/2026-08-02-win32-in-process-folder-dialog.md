...
- The driver test pins the forced Node-mode child environment, while the source plane and built `lib/worker.cjs` run under their real launchers. Automated acceptance still does not exercise a branded packaged executable opening and completing a folder selection; that remains a Windows installer acceptance gap.

...
- driver 测试固定了强制 Node 模式的子进程环境，源码路径与构建出的 `lib/worker.cjs` 则通过各自真实启动器运行。自动化验收仍未覆盖品牌打包可执行文件打开并完成文件夹选择；这仍是 Windows 安装器验收缺口。

...
`packages/host/directory-picker-native` 经 koffi——它已是仓库其他 `win32.ts` 代码的工作区依赖——在进程内打开 `IFileOpenDialog`（`FOS_PICKFOLDERS | FOS_FORCEFILESYSTEM | FOS_NOCHANGEDIR`），作为 win32 主层。COM 会话运行在 spawn 出的子进程中，模态 `Show` 永不阻塞宿主事件循环；子进程在阻塞前上报其原生线程 id，在 `Show` 阻塞期间保持 IPC 连接，并只在所选路径、取消或错误结果完成发送后断开。驱动层通过向该线程的窗口反复投递 `WM_CLOSE`（`EnumThreadWindows`）来处理中止请求，关闭等待预算耗尽后强制终止子进程。子进程环境会强制设置 `ELECTRON_RUN_AS_NODE=1`；缺少该值时，electron-builder 生成的品牌可执行文件会启动桌面入口、观察到已有单实例锁并退出，而不会加入 worker IPC 通道。koffi 通过 disposable `str16` 输出转换由 COM 持有的 UTF-16 路径，并在转换后调用 `CoTaskMemFree`；代码不使用 `koffi.view()`，因为 Electron 禁止它创建的 external ArrayBuffer。对话框是子进程的第一个窗口，Windows 会自动激活它，无需手动前台调用。子进程线程启用宿主接受的最佳线程 DPI 感知（`SetThreadDpiAwarenessContext`，按 per-monitor-v2 → per-monitor → system-aware 级联并检查返回值），严格优于脚本的系统 DPI 上限；DPI 保持为纯外观的 best-effort——不接受其中任何一种的宿主仍得到现代对话框，而不会降级。模块切分让覆盖率在任何主机上都诚实：`win32-dialog-logic.ts`（纯时序）与 `win32-dialog.ts`（driver）可在任何平台使用 fake 进行测试；`win32-dialog-bindings.ts` 对 mock 的 `koffi` COM 世界测试（`dsh-session-persistence-jsonl` 的技法）；POSIX 主机运行真实的 spawn 管道，并验证其因 koffi 加载失败而拒绝；win32 主机运行真实的打开对话框并通过中止将其关闭的冒烟测试。先于本层存在的 PowerShell 链已被删除（见[链删除](../simplification/2026-08-04-drop-windows-powershell-picker-fallback.md)）：该层无回退。
...
- binding 测试固定了不使用 external buffer 的 disposable UTF-16 输出转换，driver 测试固定了强制 Node 模式的子进程环境，worker 测试固定了 `showing` 不得在终态结果前关闭 IPC。源码路径与构建出的 `lib/worker.cjs` 则通过各自真实启动器运行。自动化验收仍未覆盖品牌打包可执行文件打开并完成文件夹选择；这仍是 Windows 安装器验收缺口。

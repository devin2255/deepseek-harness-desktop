...
已构建的 Electron 验收测试在仓库构建后运行于原生 Windows 和 macOS arm64 runner。测试临时应用数据根目录使用系统临时目录的真实路径，因此 macOS 的 `/var` 别名不会被误判为用户提供的符号链接祖先；生产日志目录仍会拒绝符号链接祖先。普通单元测试继续使用 mock Electron 边界，不会启动 Electron。
...
**在普通单元测试套件中运行 Electron。** Electron 需要原生可执行文件与测试专用用户数据隔离，不适合 source-only 单元测试 lane。真实应用作为已构建验收测试运行在原生 Windows 和 macOS arm64 CI runner 中。

## 后果

这个基础为操作系统权限提供一个明确的归属方，并且通过真实 Electron 进程证明其安全态势，包括回环授权和 Renderer 隔离。它的生命周期会等待已拥有的工作完全停稳，并报告回调故障，而不会产生未处理的 Promise 拒绝。

Electron 仍是原生 CI 依赖，因此验收测试不会运行在基于 Wine 的必需 Windows 构建通道。更新仍在这个基础之外。

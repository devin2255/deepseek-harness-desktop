...
[`dsh-base`](../packages/bundle/base/README.md) is the first layer of every profile: model adapters, tools, persistence, sandbox and approval policy, settings, credentials, telemetry. [`dsh-web-app`](../packages/bundle/web-app/README.md) adds the browser application and local Task-worktree Provider; [`dsh-desktop-app`](../packages/bundle/desktop-app/README.md) adds desktop authorization, the Task Provider, and the task overview; [`dsh-headless`](../packages/bundle/headless/README.md) adds a one-shot runner with no server at all.
...
### Task execution worktrees

`dsh-task-worktree` defines application-owned execution directories, `dsh-task-worktree-local` creates and inspects Git worktrees through `ctx.subprocess`, and `dsh-host-apiproxy` consumes the service only for an explicit `session.create` request with `isolation: worktree`. The Host creates the worktree before the Session, starts the Agent at the worktree path, and appends `task/worktree-assigned` through the Task service. Cold replay therefore restores the source Workspace identity and exact execution identity without inferring either from the current filesystem.

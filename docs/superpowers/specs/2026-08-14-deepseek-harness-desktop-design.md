...
The Harness process listens on a random loopback port and requires a high-entropy capability generated by Main for each launch. An isolated Electron session injects the authorization header only for the exact HTTP and WebSocket origin, so the capability never enters Renderer or Preload APIs; the Host also validates Origin. The capability is never written to logs, URLs, settings, or Session events. A Renderer crash does not terminate running tasks; Main detects a Harness-process crash and offers recovery in the UI instead of silently restarting it and pretending tasks are still running.
